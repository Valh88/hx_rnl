...
class Crypto
{
	...
	public static function base64Encode(data:Bytes):String
	{
		...
		Raw.RNL_base64_encode(Native.data(data), data.length, Native.charData(out), cap);
		var len = 0;
		while (len < cap - 1 && out.get(len) != 0)
			len++;
	...
	public static function base64Decode(text:String):Null<Bytes>
	{
		...
		Raw.RNL_base64_decode(Native.charData(tb), tb.length, Native.data(out), cap, Native.data(outSize), Native.data(ok));
	...
	public static function hmac(hashId:Int, key:Bytes, data:Bytes):Null<Bytes>
	{
		...
		Raw.RNL_hmac_process(hashId, Native.data(mac), Native.data(key), key.length, Native.data(data), data.length, Native.data(ok));
	...
	public static function hkdfExtract(hashId:Int, salt:Bytes, ikm:Bytes):Null<Bytes>
	{
		...
		Raw.RNL_hkdf_extract(hashId, Native.data(prk), Native.data(salt), salt.length, Native.data(ikm), ikm.length, Native.data(ok));
	...
	public static function hkdfExpand(hashId:Int, prk:Bytes, info:Bytes, outLen:Int):Null<Bytes>
	{
		...
		Raw.RNL_hkdf_expand(hashId, Native.data(out), outLen, Native.data(prk), prk.length, Native.data(info), info.length, Native.data(ok));
	...
	public static function aeadEncrypt(key32:Bytes, nonce12:Bytes, plain:Bytes, ?aad:Bytes):{cipherText:Bytes, tag:Bytes}
	{
		if (aad == null)
			aad = Bytes.alloc(0);
		var ct = Bytes.alloc(plain.length);
		var tag = Bytes.alloc(16);
		Raw.RNL_chacha20_poly1305_encrypt(Native.data(ct), Native.data(tag), Native.data(key32), Native.data(nonce12), Native.data(aad), aad.length,
	...
	public static function aeadDecrypt(key32:Bytes, nonce12:Bytes, cipherText:Bytes, tag:Bytes, ?aad:Bytes):Null<Bytes>
	{
		...
		Raw.RNL_chacha20_poly1305_decrypt(Native.data(pt), Native.data(key32), Native.data(nonce12), Native.data(tag), Native.data(aad), aad.length,
			Native.data(cipherText), cipherText.length, Native.data(ok));
	...
	public static function x25519KeyPair(rand:rnl.Random):{publicKey:Bytes, privateKey:Bytes}
	{
		var pub = Bytes.alloc(32), priv = Bytes.alloc(32), ok = Bytes.alloc(4);
		Raw.RNL_x25519_generate_public_private_key_pair(rand.native(), Native.data(pub), Native.data(priv), Native.data(ok));
		if (ok.getInt32(0) == 0)
			throw new RnlError(-1, "X25519 keygen failed");
	...
	public static function x25519SharedSecret(privKey:Bytes, remotePub:Bytes):Null<Bytes>
	{
		...
		Raw.RNL_x25519_generate_shared_secret_key(Native.data(secret), Native.data(remotePub), Native.data(privKey), Native.data(ok));
		return ok.getInt32(0) != 0 ? secret : null;
	}

	/** Generate Ed25519 signing key pair. Returns {publicKey[32], privateKey[32]}. */
	public static function ed25519KeyPair(rand:rnl.Random):{publicKey:Bytes, privateKey:Bytes}
	{
		var pub = Bytes.alloc(32), priv = Bytes.alloc(32);
		Raw.RNL_ed25519_generate_public_private_key_pair(rand.native(), Native.data(pub), Native.data(priv));
		return {publicKey: pub, privateKey: priv};
	}
	...
	public static function ed25519Sign(privKey:Bytes, pubKey:Bytes, message:Bytes):Bytes
	{
		...
		Raw.RNL_ed25519_sign(Native.data(privKey), pubKey != null ? Native.data(pubKey) : null, Native.data(message), message.length, Native.data(sig));
		return sig;
	}

	/** Ed25519 verify. */
	public static function ed25519Verify(signature:Bytes, pubKey:Bytes, message:Bytes):Bool
		return Raw.RNL_ed25519_verify(Native.data(signature), Native.data(pubKey), Native.data(message), message.length) != 0;
}
