...
class RnlEvent
{
	...
	static inline function PS():Int
		return rnl.raw.Types.PTR_BYTES;
	...
	public function new(host:Host, buf:Bytes)
	{
		...
		if (type == EventType.PeerDisconnect)
		{
			...
			for (i in 0...40)
				hex.add(StringTools.hex(buf.get(i), 2) + " ");
		...
		switch (type)
		{
			case EventType.PeerConnect, EventType.PeerDisconnect, EventType.PeerApproval, EventType.PeerDenial, EventType.PeerBandwidthLimits,
				EventType.PeerMtu, EventType.PeerReceive:
				if (hasPeer)
					peer = new Peer(host, buf, pOff);
			default:
		}
		...
		switch (type)
		{
			case EventType.PeerConnect | EventType.PeerDisconnect:
				if (buf.length >= dOff + 8)
					data = Native.getI64(buf, dOff);
			case EventType.PeerDenial:
				if (buf.length >= denOff)
					denialReason = cast buf.getInt32(denOff);
			case EventType.PeerMtu:
				if (buf.length >= mtuOff)
					mtu = Native.getU16(buf, mtuOff);
			default:
		}
	}
}
