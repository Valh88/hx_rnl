...
class Compressor
{
	...
	#if cpp
	inline function h():cpp.Star<cpp.Void>
	...
	public function compress(data:Bytes):Bytes
	{
		...
		RnlError.check(Raw.RNL_compressor_compress(h(), Native.data(data), data.length, Native.data(out), outLimit, Native.data(outSize)),
			"Compressor.compress");
	...
	public function decompress(data:Bytes, ?expectedSize:Int):Bytes
	{
		...
		var outSize = Bytes.alloc(8);
		RnlError.check(Raw.RNL_compressor_decompress(h(), Native.data(data), data.length, Native.data(out), limit, Native.data(outSize)),
	...
	function get_deflateWithHeader():Bool
		return Raw.RNL_compressor_deflate_get_with_header(h()) != 0;

	function set_deflateWithHeader(v:Bool):Bool
	{
		Raw.RNL_compressor_deflate_set_with_header(h(), v ? 1 : 0);
		return v;
	}

	public var deflateGreedy(get, set):Bool;

	function get_deflateGreedy():Bool
		return Raw.RNL_compressor_deflate_get_greedy(h()) != 0;

	function set_deflateGreedy(v:Bool):Bool
	{
		Raw.RNL_compressor_deflate_set_greedy(h(), v ? 1 : 0);
		return v;
	}
