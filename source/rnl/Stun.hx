...
class Stun {
	...
	**/
	public static function takeResult(host:Host):Null<{mappedAddress:Address, rttMs:haxe.Int64}> {
		...
		Raw.RNL_host_take_stun_result(host.h(),
			Native.data(success), Native.data(sockIdx),
			Native.data(server.native()), Native.data(mapped.native()),
			Native.data(rtt), Native.data(have));

		if (have.getInt32(0) == 0 || success.getInt32(0) == 0) return null;
